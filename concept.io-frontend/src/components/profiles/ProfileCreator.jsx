...
import {
  Checkbox, Classes, FormGroup, InputGroup, Intent,
...
function ProfileCreator({
  onNameInput, onEmailInput, onPrivateInput, onPasswordInput,
}) {
  ...
  return (
    <div className={`${styles.creator} ${Classes.DIALOG_BODY}`}>
      ...
      <FormGroup
        ...
        helperText={(invalidEmail && 'Please input an valid Email') || (duplicateEmail && 'Email is already taken.')}
        labelInfo="(required)"
      >
        <InputGroup
          id="email"
      ...
      {isPrivate && (
        <FormGroup label="Password" labelFor="passwd" labelInfo="(required)">
          <InputGroup id="passwd" type="password" placeholder="Enter your password..." onChange={handlePasswordInput} />
        </FormGroup>
      )}
    </div>
  );
...
ProfileCreator.defaultProps = {
  onNameInput: PropTypes.func,
  onEmailInput: PropTypes.func,
  onPrivateInput: PropTypes.func,
